...
PicoQuicWebTransport::PicoQuicWebTransport(
  ...
  proxygen::coro::detail::WtStreamManager::WtConfig wtConfig;
  ...
  auto dir = isClient_ ? proxygen::coro::detail::WtDir::Client
                       : proxygen::coro::detail::WtDir::Server;
  streamManager_ = std::make_unique<proxygen::coro::detail::WtStreamManager>(
...
PicoQuicWebTransport::closeSession(folly::Optional<uint32_t> error) {
  ...
  proxygen::coro::detail::WtStreamManager::CloseSession closeSession{errorCode,
                                                                     ""};
...
void PicoQuicWebTransport::onStreamData(uint64_t stream_id, uint8_t *bytes,
  ...
  proxygen::coro::detail::WtStreamManager::StreamData streamData{
      std::move(data), fin};
  auto result = streamManager_->enqueue(*readHandle, std::move(streamData));

  if (result == proxygen::coro::detail::WtStreamManager::Result::Fail) {
...
void PicoQuicWebTransport::onStreamReset(uint64_t stream_id,
                                         uint64_t error_code) {
  proxygen::coro::detail::WtStreamManager::ResetStream reset{stream_id,
                                                             error_code, 0};
...
void PicoQuicWebTransport::onStopSending(uint64_t stream_id,
                                         uint64_t error_code) {
  proxygen::coro::detail::WtStreamManager::StopSending stopSending{stream_id,
                                                                   error_code};
...
void PicoQuicWebTransport::onConnectionClose(uint64_t error_code) {
  ...
  // destroy this object
  proxygen::coro::detail::WtStreamManager::CloseSession closeSession{error_code,
                                                                     ""};
...
void PicoQuicWebTransport::processEgressEvents() {
  ...
  for (auto &event : events) {
    if (auto *resetStream =
            std::get_if<proxygen::coro::detail::WtStreamManager::ResetStream>(
                &event)) {
      int ret =
      ...
      if (ret != 0) {
        XLOG(WARN) << "Failed to reset stream " << resetStream->streamId
                   ...
                   proxygen::coro::detail::WtStreamManager::StopSending>(
                   &event)) {
      int ret =
      ...
      if (ret != 0) {
        XLOG(WARN) << "Failed to stop sending on stream "
                   ...
                   proxygen::coro::detail::WtStreamManager::CloseSession>(
                   &event)) {
